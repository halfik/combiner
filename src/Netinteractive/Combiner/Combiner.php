...
class Combiner implements CombinerInterface
{
    ...
    /**
     * Skleja i zapisju pliki do jedengo plika i zwraca url do tego pliku
     ...
     */
    static function incFile($fromPaths, $toPath, $skin = null, $extension = null)
    {
        ...
        if (\Config::get('app.debug') || !file_exists($toPath)) {
            foreach ($fromPaths as $path) {
                ...
                if (is_dir($path)) {
                    $scannedPaths = Utils::scanDir($path, '.' . $extension, true);

                    $paths = array_merge($paths, $scannedPaths);
                } else {
                    $paths[] = $path;
                }
